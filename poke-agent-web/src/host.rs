...
pub struct HostConfig {
    ...
    /// palette colours the same picture (see `gb::boot_palette`) — the red tint a real Game Boy
    ...
    /// palette RAM, and [`gb::mmu::MMU::read_sections`] re-installs the boot palette over it. Without
...
impl EmulatorHost {
    ...
    /// counted once per checkpoint. [`poke_agent::run::RunProgress`] has the argument in full.
    ...
    /// signal shuts [`gb::audio::Audio::set_output_enabled`], so the APU stops mixing and
    ...
    /// [`gb::audio::Audio::set_output_enabled`] for what stays running either way — which is
