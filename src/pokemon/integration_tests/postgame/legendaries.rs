...
/// Diagnostic for **D2a**: what can the agent actually reach on the Power Plant floor?
///
/// The eight disguised Poké Balls are `hidden`-flagged objects, so "is it there" and "can I walk to
/// it" are separate questions and only the second one shows up in `actions()`.
#[test]
#[ignore = "diagnostic — run with --ignored --nocapture"]
fn probe_power_plant() {
    let mut fixture = TestFixture::new(FLY_BIKE, Duration::from_mins(60),
        PolicyStep::electrode_steps(4).into_iter()
            .take_while(|s| !matches!(s, PolicyStep::CatchPokemon { .. })).collect());
    fixture.run_until(|s| s.map.map == Map::PowerPlant);
    dump_reachable(&mut fixture);
    let s = fixture.game_state();
    println!("sprites on the floor:");
    for sp in &s.map.sprites {
        println!("   {:<14} @ {} hidden={}", sp.name, sp.position, sp.hidden);
    }
    dump_map(&mut fixture, 0, 40, 0, 30);
}

/// **Task D2a** — an **Electrode** out of the Power Plant, carrying **Thunder Wave**.
///
/// The sub-step the re-cut §6-D turns on. All three legendary catches on record were thrown with a
/// debug-seeded Master Ball because the party's only TM45-compatible member is a lv30 Slowpoke, and a
/// paralyser slower than Moltres cannot act through Fire Spin at all. Electrode is the one obtainable
/// answer: base speed **140**, and it learns TM45.
///
/// Rooted on **B's output** rather than D1a's, and that is the point — a Gen 1 TM is consumed on use
/// and TM45 is one per cartridge, so the Electrode has to be caught *before* the TM is picked up.
///
/// ⚠️ **Measured: both Electrodes Selfdestruct and the sub-step is lost.** This is the D2a blocker,
/// written up in §11 with the arithmetic. The route, the engagement and the TM pickup all work — what
/// does not is the fight, at roughly a **31 % coin flip per Electrode and only two on the cartridge**.
/// Kept runnable rather than deleted, because everything except the last coin flip is verified and a
/// future fix (a sleep move, or weakening to the 25 % HP point where Ultra Balls saturate the second
/// roll) needs exactly this leg to test against.
#[test]
#[ignore = "known blocker — both Electrodes Selfdestruct; see docs/postgame-coverage-plan.md §11 (D2a)"]
fn can_catch_an_electrode() {
    /// It appends after the four-mon party.
    const ELECTRODE_SLOT: u8 = 4;

    let mut fixture = TestFixture::new(FLY_BIKE, Duration::from_mins(90),
        PolicyStep::electrode_steps(ELECTRODE_SLOT));

    let before = fixture.game_state();
    assert!(!before.pokedex_owned.contains(&PokemonSpecies::Electrode), "already owns one");
    assert_eq!(before.pokemon.len(), 4, "the Electrode is expected to land in slot 4");

    let caught = fixture.run_until(|s| s.pokedex_owned.contains(&PokemonSpecies::Electrode));
    println!("Electrode caught at {:?}", caught.map.map);
    assert_eq!(caught.pokemon[ELECTRODE_SLOT as usize].species, PokemonSpecies::Electrode);

    let state = fixture.run_leg(|s| s.pokemon.get(ELECTRODE_SLOT as usize).is_some_and(|p|
        p.moves.iter().flatten().any(|m| m.name == PokemonMoveName::ThunderWave)));
    // Speed is the whole reason this Pokémon and not the Slowpoke: it has to move before Fire Spin.
    println!("Electrode lv{} speed {} knows {:?}",
        state.pokemon[ELECTRODE_SLOT as usize].level,
        state.pokemon[ELECTRODE_SLOT as usize].stats.speed,
        state.pokemon[ELECTRODE_SLOT as usize].moves.iter().flatten()
            .map(|m| m.name).collect::<Vec<_>>());
    assert!(state.pokemon[ELECTRODE_SLOT as usize].stats.speed
        > state.pokemon[0].stats.speed, "the point of the Electrode is that it outspeeds");

    fixture.save_state_named("src/pokemon/data/postgame-electrode.bin").unwrap();
}
