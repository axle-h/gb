# 7 badges: win the Earth Badge

Goal: beat Giovanni in `ViridianGym`, the last gym. It is back where the game started.

Route: Fly to `ViridianCity` > `ViridianGym`.

1. `ViridianGym` has been shut all game and says so: "The GYM's doors are locked...".
...
4. Giovanni has Rhyhorn 45, Dugtrio 42, Nidoqueen 44, Nidoking 45 and Rhydon 50. They
   ...
   for double, and all three of those run off Special, which is the stat his team is
   weakest in. Watch for Horn Drill and Fissure: both are one-hit knockouts, and both
   miss far more often than they land.

The Earth Badge makes every traded Pokémon obey you whatever its level.

That is all eight. The Pokémon League is open. Heal, then head west out of
`ViridianCity` on to `Route22`, where the rival is waiting one last time before
`Route23`. Take Pokémon that know Surf and Strength: the road there needs both.
