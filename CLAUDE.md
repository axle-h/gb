...
environment block, the build and the deployment. This file has only what it leaves out: the
invariants, the traps, and the test workflows. Nearly every ⚠️ was learned by breaking something.

The only surviving design doc is `docs/llm-web-playthrough-plan.md` (W0–W9, all done). Where a
number below is attributed to "Phase C" or "Phase D", that is history, not a file you can open.
...
- **No em dashes in anything a viewer or a model reads** — see the `Display` ⚠️ under "Agent and
  policy invariants". The punctuation in *this* file and in code comments is a different audience.
...
`tune_audio` under "The audio stream".)
...
**The one exception is the shape of the rule, not a hole in it**: the park on a spent quota (below) happens when a
...
fixture lives in `src/pokemon/data/gfx/`, not `src/pokemon/data/`** — see the fixture-directory ⚠️ above.
...
# ⚠️ The kbit/s in the README and in this file come from these two and nowhere else.
...
Because it is opt-in and slow, it rotted once already: it sat broken while its own doc comment, this file and the plan all
