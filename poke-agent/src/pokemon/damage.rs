...
/// Returns `true` if `name` is a move that deals direct damage. Mirrors the special cases at the
/// top of `expected_damage` in `damage.rs` as well as ordinary moves that have a positive base
/// power.
...
/// Returns approximate damage dealt by `move_name` used by `attacker` against `defender`, or
/// `None` if the move cannot deal damage (zero power, or the defender is immune).
pub fn expected_damage(attacker: &PokemonSummary, move_name: PokemonMoveName, defender: &PokemonSummary) -> Option<u16> {
    ...
    // Calculating A and D: For special moves, substitute Attack and Defense for Special.
    ...
    // No critical hit, so use the modified stats (here treated as unmodified since we have no
    // in-battle stage information), with no Reflect/Light Screen doubling of D.
    let (mut a, mut d) = (a as u32, d as u32);

    // If A or D >= 256, divide both by 4 and reduce modulo 256.
    if a >= 256 || d >= 256 {
        a = (a / 4) % 256;
        d = (d / 4) % 256;
        if a == 0 { a = 1; }
        ...
        .filter_map(|a| match a {
            BattleAction::Fight { battle_move, .. } => {
                ...
                if dmg > 0 && (!catching_pokemon || dmg < battle_state.enemy.current_hp) {
                    // Ranked per turn, but the *catching* guard above stays on the raw number:
...
/// [`type_multiplier`] as the cartridge's own words for it, or `None` at 1.0 — where the game
/// says nothing and so does this.
